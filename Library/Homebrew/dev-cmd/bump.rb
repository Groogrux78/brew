    ...
    class Bump < AbstractCommand
      ...
      def version_with_cooldown(version_info, current = nil)
        ...
        when "Npm"
          ...
          return unless (release_dates = json["time"])

          cooldown_interval = (DateTime.now - COOLDOWN_DAYS)
          release_dates.sort_by { |k, _| Version.new(k) }.reverse_each do |version, date_str|
            date = Date.parse(date_str)
            return Version.new(version) if date < cooldown_interval
          end
        when "Pypi"
          url = version_info.dig(:meta, :url, :strategy)
          suffix = Homebrew::Livecheck::Strategy::Pypi::URL_MATCH_REGEX.match(
            version_info.dig(:meta, :url, :original),
          )&.[](:suffix)
          ...
          cooldown_interval = (DateTime.now - COOLDOWN_DAYS)
          releases.sort_by { |k, _| Version.new(k) }.reverse_each do |version, assets|
